...
class SvDxfCirclesNode(SverchCustomTreeNode, bpy.types.Node):
    bl_idname = 'SvDxfCirclesNode'
    bl_label = 'DXF Circles'
    bl_icon = 'EXPORT'
    ...
    def process(self):
        if self.outputs['dxf'].is_linked and self.inputs['curves'].is_linked:
            # All starts with dxf socket
            curvs_ = self.inputs['curves'].sv_get()
            ...
            lt = self.linetype
            for obc,met in zip_long_repeat(curvs_,meta_):
                if type(obc) in ['list','tuple']:
                    for c in obc:
                        curv = DxfCircles(c,cols_,lw,met[0],lt,self,color_int)
                        #print('iterate circles in object')
                        dxf.append(curv)
                else:
                    curv = DxfCircles(obc,cols_,lw,met[0],lt,self,color_int)
                    dxf.append(curv)
            self.outputs['dxf'].sv_set([dxf])

    def sv_update(self):
        if not ("curves" in self.inputs):
            return

def register():
    bpy.utils.register_class(SvDxfCirclesNode)

def unregister():
    bpy.utils.unregister_class(SvDxfCirclesNode)
