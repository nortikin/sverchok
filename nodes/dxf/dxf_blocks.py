...
from sverchok.nodes.dxf.dxf_export import DXFExportOperator
if ezdxf != None:
    import ezdxf
...
class SvDxfBlocksNode(SverchCustomTreeNode, bpy.types.Node):
    bl_idname = 'SvDxfBlocksNode'
    bl_label = 'DXF Blocks'
    bl_icon = 'EXPORT'
    bl_category = 'DXF'
    ...
    def draw_buttons(self, context, layout):
        ...
        row.scale_y = scale_y
        row.operator("node.dxf_export", text="Export DXF block")
...
def register():
    bpy.utils.register_class(SvDxfBlocksNode)

def unregister():
    bpy.utils.unregister_class(SvDxfBlocksNode)
