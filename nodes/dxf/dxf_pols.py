...
class SvDxfPolygonsNode(SverchCustomTreeNode, bpy.types.Node):
    bl_idname = 'SvDxfPolygonsNode'
    bl_label = 'DXF Polygons'
    bl_icon = 'EXPORT'
    sv_icon = 'SV_MESH_SVG'
    bl_category = 'DXF'
        ...
        if self.outputs['dxf'].is_linked:
            ...
            for obv,obp,met in zip_long_repeat(vers_,pols_,meta_):
                ...
                for po, me in zip_long_repeat(obp,met):
                    ...
                    pols = DxfPols(points,cols_,lw,me,lt,self)
                    dxf.append(pols)
...
def register():
    bpy.utils.register_class(SvDxfPolygonsNode)

def unregister():
    bpy.utils.unregister_class(SvDxfPolygonsNode)
