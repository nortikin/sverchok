...
class SvDxfTextsNode(SverchCustomTreeNode, bpy.types.Node):
    bl_idname = 'SvDxfTextsNode'
    bl_label = 'DXF Text'
    bl_icon = 'EXPORT'
    sv_icon = 'SV_TEXT_SVG'
    bl_category = 'DXF'
    ...
    text_scale: bpy.props.FloatProperty(default=250.0,name='text_scale')
    ...
    def process(self):
        if self.outputs['dxf'].is_linked and self.inputs['verts'].is_linked and self.inputs['text'].is_linked:
            ...
            for obv, tex, met in zip_long_repeat(vers_,text_,meta_):
                # объекты
                for ve, tx, me in zip_long_repeat(obv,tex,met):
                    # каждый полигон это отдельный экземпляр
                    #print('packing text:',ve,tx)
                    texts = DxfTexts([ve,tx],cols_,lw,me,lt,text_scale,self,color_int)
                    dxf.append(texts)
...
def register():
    bpy.utils.register_class(SvDxfTextsNode)

def unregister():
    bpy.utils.unregister_class(SvDxfTextsNode)
