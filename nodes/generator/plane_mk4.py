...
class SvPlaneNodeMk4(DraftMode, bpy.types.Node, SverchCustomTreeNode):
    """
    Triggers: Grid,
    Tooltip: Generate a Plane primitive.
    """

    bl_idname = 'SvPlaneNodeMk4'
    bl_label = 'Plane'
    bl_icon = 'MESH_PLANE'
    ...
    def update_sockets(self, context):
        ...
        if self.dimension_mode in ('STEPS', 'SIZE_STEPS'):
            ...
            self.inputs['Step X'].prop_name = f'step{first_axis.lower()}'
            self.inputs['Step Y'].prop_name = f'step{second_axis.lower()}'
            hide('Num X', False)
            hide('Num Y', False)
    ...
    def update_properties(self):
        ...
        if self.inputs['Step X'].prop_name:
            self.inputs['Step X'].prop_name = f'step{first_axis.lower()}'
            self.inputs['Step Y'].prop_name = f'step{second_axis.lower()}'

        self.inputs['Size X'].label = f'Size {first_axis}'
        self.inputs['Size Y'].label = f'Size {second_axis}'
        self.inputs['Size X'].prop_name = f'size{first_axis.lower()}'
        self.inputs['Size Y'].prop_name = f'size{second_axis.lower()}'

        self.inputs['Num X'].label = f'Num {first_axis}'
        self.inputs['Num Y'].label = f'Num {second_axis}'
        self.inputs['Num X'].prop_name = f'num{first_axis.lower()}'
        self.inputs['Num Y'].prop_name = f'num{second_axis.lower()}'
    ...
    numz: IntProperty(
        name='N Verts Z', description='Number of vertices along Z',
        default=2, min=2, update=updateNode)
    ...
    numz_draft: IntProperty(
        name='[D] N Verts Z', description='Number of vertices along Z (draft mode)',
        default=2, min=2, update=updateNode)
    ...
    stepz: FloatProperty(
        name='Step Z', description='Step length Z',
        default=1.0, update=updateNode)
    ...
    stepz_draft: FloatProperty(
        name='[D] Step Z', description='Step length Z (draft mode)',
        default=1.0, update=updateNode)
    ...
    sizez: FloatProperty(
        name='Size Z', description='Plane size along Z',
        default=10.0, min=0.01, update=updateNode)
    ...
    def migrate_from(self, old_node):
        if old_node.direction == 'YZ':
            self.numy = old_node.numx
            self.numz = old_node.numy
            self.numy_draft = old_node.numx_draft
            self.numz_draft = old_node.numy_draft
            self.stepy = old_node.stepx
            self.stepz = old_node.stepy
            self.stepy_draft = old_node.stepx_draft
            self.stepz_draft = old_node.stepy_draft
            self.sizey = old_node.sizex
            self.sizez = old_node.sizey
            self.sizey_draft = old_node.sizex_draft
            self.sizez_draft = old_node.sizey_draft
        elif old_node.direction == 'ZX':
            self.numz = old_node.numx
            self.numx = old_node.numy
            self.numz_draft = old_node.numx_draft
            self.numx_draft = old_node.numy_draft
            self.stepz = old_node.stepx
            self.stepx = old_node.stepy
            self.stepz_draft = old_node.stepx_draft
            self.stepx_draft = old_node.stepy_draft
            self.sizez = old_node.sizex
            self.sizex = old_node.sizey
            self.sizez_draft = old_node.sizex_draft
            self.sizex_draft = old_node.sizey_draft
        self.update_properties()
    ...
    def get_data(self):
        ...
        if self.dimension_mode == 'SIZE':
            ...
            params = [inputs['Step X'].sv_get(deepcopy=False, default=[[1.0]]), inputs['Step Y'].sv_get(deepcopy=False, default=[[1.0]])]
        else:
            params=[inputs['Step X'].sv_get(deepcopy=False, default=[[1.0]])]
            params.append(inputs['Step Y'].sv_get(deepcopy=False, default=[[1.0]]))
        ...
        mat_input = inputs['Matrix'].sv_get(deepcopy=False, default=[[Matrix()]])
...
def register():
    bpy.utils.register_class(SvPlaneNodeMk4)


def unregister():
    bpy.utils.unregister_class(SvPlaneNodeMk4)
