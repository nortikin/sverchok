...
from sverchok.utils.intersect_edges import intersect_edges_3d, intersect_edges_2d, remove_doubles_from_edgenet, intersect_edges_2d_np, intersect_edges_2d_np_big
...
class SvIntersectEdgesNodeMK2(bpy.types.Node, SverchCustomTreeNode):
    ...
    def process(self):
        ...
        for vs, eds in zip(verts_in, edges_in):
            if self.mode == "3D":
                v_out, ed_out = intersect_edges_3d(vs, eds, 1 / 10 ** self.epsilon)

            elif self.alg_mode_2d == "Alg_1":
                if len(eds) > self.big_data_limit:
                    v_out, ed_out = intersect_edges_2d_np_big(vs, eds)
                else:
                    v_out, ed_out = intersect_edges_2d_np(vs, eds)
            ...
            if self.rm_switch and (self.mode == "3D" or self.alg_mode_2d in ['Alg_1']):
