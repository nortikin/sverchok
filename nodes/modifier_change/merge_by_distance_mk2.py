...
class SvMergeByDistanceNodeMk2(bpy.types.Node, SverchCustomTreeNode, SvRecursiveNode):
    """
    Triggers: Remove Doubles
    Tooltip: Merge Vertices that are closer than a distance.
    """
    bl_idname = 'SvMergeByDistanceNodeMk2'
    bl_label = 'Merge by Distance2'
    ...
    def sv_init(self, context):
        ...
        poly_edge = self.inputs.new('SvStringsSocket', 'PolyEdge')
        poly_edge.nesting_level = 3
        poly_edge.default_mode = 'EMPTY_LIST'

        face_data = self.inputs.new('SvStringsSocket', 'FaceData')
        ...
        distance = self.inputs.new('SvStringsSocket', 'Distance')
        distance.prop_name = 'distance'
...
def register():
    bpy.utils.register_class(SvMergeByDistanceNodeMk2)


def unregister():
    bpy.utils.unregister_class(SvMergeByDistanceNodeMk2)
