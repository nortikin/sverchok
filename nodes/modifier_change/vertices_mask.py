...
class SvVertMaskNode(ModifierLiteNode, SverchCustomTreeNode, bpy.types.Node):
    ...
    def process(self):
        ...
        if not (self.inputs["Poly Egde"].is_linked):
            raise Exception(f"Input socket '{self.inputs['Poly Egde'].label or self.inputs['Poly Egde'].identifier}' has to be connected")
