...
class SvOffsetLineNode(bpy.types.Node, SverchCustomTreeNode):
    ...
    def process(self):
        ...
        verts_in = self.inputs['Vers'].sv_get()[0]
        edges_in = self.inputs['Edgs'].sv_get()[0]
        shift = self.inputs['Offset'].sv_get()[0][0]
        ...
        self.outputs['OuterEdges'].sv_set([outer_edges])
        self.outputs['VersMask'].sv_set([vers_mask])
