...
from sverchok.utils.surface import SvExSurface, SvExSurfaceSubdomain

class SvExSurfaceSubdomainNode(bpy.types.Node, SverchCustomTreeNode):
    """
    ...
    def process(self):
        ...
        for surfaces, u_mins, u_maxs, v_mins, v_maxs in zip_long_repeat(surface_s, u_min_s, u_max_s, v_min_s, v_max_s):
            for surface, u_min, u_max, v_min, v_max in zip_long_repeat(surfaces, u_mins, u_maxs, v_mins, v_maxs):
                new_surface = SvExSurfaceSubdomain(surface, (u_min, u_max), (v_min, v_max))
                surface_out.append(new_surface)

        self.outputs['Surface'].sv_set(surface_out)

def register():
    bpy.utils.register_class(SvExSurfaceSubdomainNode)

def unregister():
    bpy.utils.unregister_class(SvExSurfaceSubdomainNode)
