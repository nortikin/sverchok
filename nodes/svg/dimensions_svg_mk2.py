...
class SvgDimension():
    ...
    def __init__(self, location, location_b, dim_type, dim_offset, font_size, text_offset, text_multiplyer, text_attrib, lines_attrib, font_family, node):
        self.node = node
        ...
        self.text_multiplyer = text_multiplyer
    ...
    def draw(self, document):
        ...
        text_svg = self.draw_dimension_text(dim_loc_a, dim_loc_b, angle, document, self.text_multiplyer)
        return lines_svg + text_svg

    def draw_dimension_text(self, dim_loc_a, dim_loc_b, angle, document, multiplyer):
        height = document.height
        scale = document.scale
        ...
        if self.node.units_real == 'Metric':
            ...
            text_svg += p_format.format((dim_loc_b - dim_loc_a).length*multiplyer)
        else:
            feets = multiplyer*(dim_loc_b - dim_loc_a).length//0.3048
            if feets < 2:
                inches = (multiplyer*(dim_loc_b - dim_loc_a).length/0.0254)
            else:
                p_format = "{:10.0f}"
                inches = (multiplyer*((dim_loc_b - dim_loc_a).length%0.3048)/0.0254)
...
class SvSvgDimensionNodeMK2(SverchCustomTreeNode, bpy.types.Node):
    """
    Triggers: Text SVG
    Tooltip: Creates SVG Dimensions
    """
    bl_idname = 'SvSvgDimensionNodeMK2'
    ...
    units_real: EnumProperty(
        ...
        items=enum_item_4(['Metric', 'Imperial']),
        default='Metric',
        update=updateNode)
    ...
    text_multiplyer: FloatProperty(
        name='Text Multiplyer',
        description='Text multiplyer',
        default=1.0,
        update=updateNode)
    ...
    def sv_init(self, context):
        ...
        self.inputs.new('SvStringsSocket', "Text Multiplyer").prop_name = 'text_multiplyer'
    ...
    def process(self):
        ...
        if not self.outputs[0].is_linked:
            return
        params_in = [s.sv_get(deepcopy=False) for s in self.inputs[:7]]
        texts_out = []
...
def register():
    bpy.utils.register_class(SvSvgDimensionNodeMK2)


def unregister():
    bpy.utils.unregister_class(SvSvgDimensionNodeMK2)
