...
from sverchok.data_structure import (fullList, match_long_repeat,dataCorrect, repeat_last, updateNode, match_long_cycle)
from sverchok.utils.sv_mesh_utils import mesh_join
from sverchok.utils.cad_module_class import CAD_ops
from sverchok.utils.sv_bmesh_utils import bmesh_from_pydata

modeItems = [("Constant", "Constant", ""), ("Weighted", "Weighted", "")]
listMatchItems = [("Long Repeat", "Long Repeat", ""), ("Long Cycle", "Long Cycle", "")]
...
def distanceEdges(v1,v2,p):
    return abs((v2[1]-v1[1])*p[0]-(v2[0]-v1[0])*p[1]+(v2[0]*v1[1])-(v2[1]*v1[0]))/pow(pow(v2[1]-v1[1],2)+pow(v2[0]-v1[0],2),0.5)
...
class SvIsoLinesNode(bpy.types.Node, SverchCustomTreeNode):
    ''' 2D Path with offset '''
    bl_idname = 'SvIsoLinesNode'
    bl_label = 'Isoline 2D'
    bl_icon = 'MESH_CIRCLE'

    modeI = EnumProperty(
        name="modeI", description = "Constant or weigted distance when multiple radius are given",
        items=modeItems, default="Constant", update=updateNode)
    listMatch = EnumProperty(
        name="listMatch", description = "Behaviour on diffent list lengths",
        items=listMatchItems, default="Long Cycle", update=updateNode)
    
    rm_doubles = bpy.props.FloatProperty(
        description = "Remove Doubles Distance",
        min=0.0, default=0.0001, step=0.1, update=updateNode)
    epsilon = bpy.props.FloatProperty(
        description = "Intersection tolerance",
        min=1.0e-5, default=1.0e-5, step=0.02, update=updateNode)
    maskT = bpy.props.FloatProperty(
        description = "Mask tolerance",
        min=-1.0, default=1.0e-5, step=0.02, update=updateNode)
    ...
    def draw_buttons(self, context, layout):
        ...
        layout.prop(self, 'rm_doubles', text='R. Doubles')

    def draw_buttons_ext(self, context, layout):
        layout.prop(self, "modeI", expand = True)
        layout.prop(self, 'rm_doubles', text = 'R. Doubles')
        layout.prop(self, 'epsilon', text = 'Int. Tolerance')
        layout.prop(self, 'maskT', text = 'Mask tolerance')
...
def register():
    bpy.utils.register_class(SvIsoLinesNode)


def unregister():
    bpy.utils.unregister_class(SvIsoLinesNode)
