...
If you look carefully in ``sv_main`` there's not a lot to the whole process. ``sv_main`` has two **required** arrays; ``in_sockets`` and ``out_sockets``. The lorenz function takes 1 argument, N, to set the number of vertices. I've also passed in the ``verts`` list-variable to store the vertices generated by the algorithm.
