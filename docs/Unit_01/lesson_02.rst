...
5) before i can show you 5, we'll have to explain a few things. View the output of the ``Number Range`` socket using stethoscope. you'll see ``[[0,1,2,3]]``
  ...
  Sverchok sockets can pass multiple items, per socket. They can contain the vertices of any number of objects. The number of objects is marked with a number beside the socket name. 
