...
3) Leaving Vector In's 3rd socket (``z``) empty puts a ``0.0`` as the ``z`` component for all vectors generated by that node.
  ...
  *Note*: Astute readers will have notices that we connected a ``Vector socket`` (orange) to a ``"Numbers" socket`` (green). We allow any connection between socket types that can be made to work. There are automatic conversions going on in the background. We call them implicit conversions.
