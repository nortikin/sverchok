...
`A, B, C and D` are all different ways of thinking about the same data in the socket::
  ...
  # 3. The ``Brackets`` are what encapsulate the vertices associated with one object.
  # 4. The outer most brackets are what collect all sublists into something that we can pass through a socket.
  [
