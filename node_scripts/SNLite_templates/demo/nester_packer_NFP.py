...
self.make_operator('test')


def ui(self, context, layout):
    cb_str = 'node.scriptlite_custom_callback'
    layout.operator(cb_str, text='P A C K').cb_name='test'
...
def test(self, context):
    ...
    if FreeCAD is not None:
        import Part
        from FreeCAD import Base # DraftGeomUtils, WorkingPlane, DraftVecUtils, Draft
    ...
    cont_ver = self.inputs['cont_ver'].sv_get()[0]
    cont_pol = self.inputs['cont_pol'].sv_get()[0]
    vers = self.inputs['vers'].sv_get()
    pols = self.inputs['pols'].sv_get()
    #print('Container',cont_ver,cont_pol)
    def make_faces(verts, face_idxs):
        ...
        return result
    container = make_faces(cont_ver,cont_pol)
    #container = BFPD(cont_ver, [], cont_pol, normal_update=True)
    print('container',container)
    #print('Shapes',vers,pols)
    shapes = [make_faces(v,p)[0] for v,p in zip (vers,pols)]
