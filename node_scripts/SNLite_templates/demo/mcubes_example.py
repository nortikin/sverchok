...
import os
try:
    mcubes_path = r"/usr/local/lib/python3.9/dist-packages" #it depend on your OS but just paste the path where is mcubes
    if not mcubes_path in sys.path:
        sys.path.append(mcubes_path)    
    import mcubes
except:
    os.system('pip3.9 install pymcubes')
# Create the volume
def f(x, y, z):
    return (z**2 / (math.sin(z*y+x)) + 3**x)**2
