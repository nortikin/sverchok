...
class IFC_export:
    ...
    def __init__(self, vers, pols, node, mode, path, base_name, max_faces, area_threshold):
        self.vers = vers
        self.pols = pols
        self.node = node
        self.mode = mode
    ...
    def clean_ascii_string(self, text):
    ...
    def prepare(self, context):
        ...
        vertices_list = self.vers
        polygons_list = self.pols
        max_faces = self.max_faces
        area_threshold = self.area_threshold
        geometry_type = self.mode
        ...
        base_filepath = self.path


        # Обрабатываем имена объектов
        base_object_name = self.clean_ascii_string(self.base_name)
        ...
        try:
            ...
            ifc_content = self.generate_ifc_content_multiple(objects_data, base_object_name,max_faces)
    ...
    def ensure_area(self, points, indices):
        ...
        if area < self.area_threshold:  # Порог для вырожденности
            return False
        else:
            return True

    def ensure_counter_clockwise(self, points, indices):
    ...
    def are_points_collinear(self, points, epsilon=1e-6):
    ...
    def triangulate_polygon_simple(self, points, indices):
        ...
        for i in range(1, n-1):
            area = self.ensure_area(points, [indices[0], indices[i], indices[i+1]])
            if area:
    ...
    def create_geometry_entities_for_object(self, verts, polys, start_index, obj_index, max_faces, geometry_type="BREP"):
        """
        ...
        for poly in polys[:max_faces]:
            ...
            if len(valid_poly) >= 3:
                if not self.are_points_collinear([unique_verts[i] for i in valid_poly[:3]]):
                    valid_poly = self.ensure_counter_clockwise(verts, valid_poly)
                    valid_polys.append(valid_poly)
        
        # 4. Выбор типа геометрии
        if geometry_type == "BREP":
            entities_part, obj_data = self.create_brep_geometry(entities, obj_data, unique_verts, valid_polys, current_index)
        else:  # MESH
            entities_part, obj_data = self.create_triangulated_mesh_geometry(entities, obj_data, unique_verts, valid_polys, to_triangle, current_index)
    ...
    def create_brep_geometry(self, entities, obj_data, unique_verts, valid_polys, current_index):
        """Создание геометрии в формате FacetedBrep (оболочки из граней)"""
        
        # Создаем треугольники для FacetedBrep
        for poly in valid_polys:
            triangles = self.triangulate_polygon_simple(unique_verts, poly)
    ...
    def create_triangulated_mesh_geometry(self, entities, obj_data, unique_verts, valid_polys, to_triangle, current_index):
    ...
    def generate_ifc_content_multiple(self, objects_data, base_obj_name,max_faces):
        ...
        project_guid = self.create_guid(num=22)
        site_guid = self.create_guid(num=22)
        building_guid = self.create_guid(num=22)
        storey_guid = self.create_guid(num=22)
        ...
        for obj_idx, obj_data in enumerate(objects_data):
            ...
            geometry_str, obj_geometry_data = self.create_geometry_entities_for_object(
        ...
        for elem in element_data:
            ...
            elem_guid = self.create_guid(num=22)
            additional_guid = self.create_guid(num=36)
        ...
        site_name = self.clean_ascii_string(f"Site_{base_obj_name}")
        building_name = self.clean_ascii_string(f"Building_{base_obj_name}")
        storey_name = "Level_0"
        allen = "\n".join(all_entities)
...
#{current_index+2}=IFCBUILDINGSTOREY('{storey_guid}',#5,'{storey_name}',$,$,$,$,$,.ELEMENT.,0.);"""
        ...
        rel_guid1 = self.create_guid(num=22)
        rel_guid2 = self.create_guid(num=22)
        rel_guid3 = self.create_guid(num=22)
        rel_guid4 = self.create_guid(num=22)
