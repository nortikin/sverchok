...
def nearest_point_on_curve(src_points, curve, samples=10, precise=True, method='Brent', output_points=True, logger=None):
    ...
    def init_guess(curve, points_from):
        ...
        for point_from in points_from:
            ...
            # Если во время расчётов выяснилось, что точка пересечения не соответствует своему сегменту,
            ...
            # 6 time is appoximating max count of attemps to count accurate point as nearest point.
            ...
            for iter_over_segment in range(6):
                ...
                # расстояние по максимальной оси от nearest до p0: https://user-images.githubusercontent.com/14288520/212532480-89b93d9d-7019-4f58-95a7-76ef537a2001.png
                # what distance to p0 by axis is max?
                dist_nearest_to_p0 = nearest_p0[abs(nearest_p0).argmax()] 

                nearest_point = None
                us0 = us[segment_i]
                # Никогда не будет превышать массив samples, т.к. количество сегментов всегда меньше количества точек.
