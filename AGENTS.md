...
**Sverchok** is a parametric geometry programming tool for Blender, enabling
visual programming of geometry through nodes.
...
- **Additional libraries**: scipy, geomdl, skimage, mcubes, circlify, cython,
  numba, numexpr, ezdxf, pyacvd, pySVCGAL, spyrrow, freecad
...
**IMPORTANT**: Project, root directory, and root module are called `sverchok`,
not `sverzok`! Imports follow the pattern:
...
1. **Root modules**: `dependencies`, `data_structure`, `node_tree`, `core`,
   `utils`, `ui`, `nodes`, `old_nodes`
2. **Core modules**: `sv_custom_exceptions`, `update_system`, `sockets`,
   `socket_data`, `handlers`, `events`, `node_group`, `tasks`,
   `group_update_system`, `event_system`, `socket_conversions`

The `core/__init__.py` file contains the `init_architecture()` function that orchestrates module initialization.

### Registration System

Node modules are automatically discovered and registered in `nodes/__init__.py`
via `automatic_collection()` which walks the directory tree. Each subdirectory
under `nodes/` becomes a node category.
...
Solid nodes use `sv_category` for sub-categorization (e.g., `"Solid
Operators"`, `"Solid Inputs"`, `"Solid Outputs"`).
...
`self.inputs.new(SocketClass, "Name")` and `self.outputs.new(SocketClass,
"Name")`.
...
All custom sockets inherit from Blender's `NodeSocket` via `SvBaseSocket`.

### Data Processing & Data Flow

Sverchok uses a **batched data model**: data is always a **list of lists** (or
deeper nesting). Each outer list element = one "item" or "batch".
...
5. **Register/unregister** the class (or use `register_classes_factory`)
6. **Add tests** in `tests/`
7. Ensure the module is imported in the category's `__init__.py`

Nodes are auto-discovered via `automatic_collection()` in `nodes/__init__.py`.
...
2. **Documentation**: Use docstrings for all functions and classes (`Triggers:`
   for search, `Tooltip:` for hover)
...
6. **Focus**: Keep node classes focused — complex algorithms belong in `utils/`
7. **License header**: Include the Sverchok GPL3 license header in new files
8. **Icons**: Use `sv_icon` for custom icons; fall back to `bl_icon` for
   standard icons
...
1. **Missing dependency check**: Always check if a dependency is `None` before
   using it
2. **Data nesting**: Use `ensure_nesting_level()` when input data may have
   varying nesting depths
3. **Early exit**: Always check `any(socket.is_linked for socket in
   self.outputs)` at the start of `process_node()`
...
Sverchok implements NURBS curve operations with two backends (both inherit from
`SvNurbsCurve` in `utils/curve/nurbs.py`):
...
Themes: `Sverchok_light_3`, `Sverchok_light_4`, `Sverchok_light_5` (per Blender
version). Themes are auto-applied. Custom colors can be set in preferences.

## Legacy Nodes

The `old_nodes/` directory contains deprecated nodes maintained for backward
compatibility. When refactoring:
...
Documentation is built using Sphinx (`docs/` directory, `./build_docs.sh`).
Node documentation is auto-generated from docstrings.
